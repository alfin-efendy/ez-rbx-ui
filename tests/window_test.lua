...
h.describe("window", function()
  ...
  h.it("acrylic blur toggles with window visibility", function()
    local R = h.loadLib()
    local w = newWin()
    h.expect(w.AcrylicBlur.Instance.Enabled).toBe(true)
    w:Hide(); h.expect(w.AcrylicBlur.Instance.Enabled).toBe(false)
    w:Show(); h.expect(w.AcrylicBlur.Instance.Enabled).toBe(true)
  end)
