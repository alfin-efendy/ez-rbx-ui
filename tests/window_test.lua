...
h.describe("window", function()
  ...
  h.it("simple floating toggle is a labeled pill (matches the old version)", function()
    local R = h.loadLib(); local screen = h.roblox.Instance.new("ScreenGui"); R.Overlay.get(screen)
    local w = R.Window.new({ Title = "MyHub", Parent = screen, FloatingToggle = true })
    local fab; for _, c in ipairs(R.Overlay.get(screen):GetChildren()) do if c.Name == "FloatingToggle" then fab = c end end
    h.expect(fab:GetAttribute("FabType")).toBe("simple")
    local lbl = fab:FindFirstChildOfClass("TextLabel")
    h.expect(lbl ~= nil).toBeTruthy()
    h.expect(tostring(lbl.Text):find("Open") ~= nil).toBeTruthy()
  end)
