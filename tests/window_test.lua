...
h.describe("window", function()
  ...
  h.it("solid darker window + solid lighter content card (no acrylic)", function()
    local R = h.loadLib()
    local w = newWin()
    -- window backing: darker chrome colour, rounded, SOLID (no acrylic transparency)
    h.expect(w.Main.BackgroundColor3.R8).toBe(R.Theme.Colors.background.R8)
    h.expect(w.Main.BackgroundTransparency).toBe(0)
    ...
    -- content panel: SOLID lighter card, rounded inset, borderless
    h.expect(panel.BackgroundColor3.R8).toBe(R.Theme.Colors.card.R8)
    h.expect(panel.BackgroundTransparency).toBe(0)
    ...
    h.expect(w.SetAcrylicTransparency).toBe(nil)  -- acrylic feature removed
